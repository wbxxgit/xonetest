...
//	unsigned char s_talk_data[2048] = {0};	
	...
	while(1){		
		...
		if(stH_glget_audio()->iRun == 1){		
			DPRI_INFO("audio iRun openflag:%d \n",stH_glget_audio()->openflag);
			...
			//		DPRI_INFO("SDKCMD_INTERCOM_START fail\n");					
				DPRI_INFO("intercom play over.openflag[%d]\n",stH_glget_audio()->openflag);							
			} 	 
			usleep(200*000);
		}	
	}
