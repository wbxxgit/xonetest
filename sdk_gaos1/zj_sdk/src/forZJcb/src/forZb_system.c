...
//	unsigned int timezone = 0;
	...
	time(&t1);
	t2 = t1;
	DPRI_INFO("t1=%ld,t2=%ld\n", t1, t2);
	tm_local = localtime(&t1);
	DPRI_INFO("localtime=%d:%d:%d\n", tm_local->tm_hour, tm_local->tm_min, tm_local->tm_sec);
	t1 = mktime(tm_local) ;
	tm_utc = gmtime(&t2);
	DPRI_INFO("utcutctime=%d:%d:%d\n", tm_utc->tm_hour, tm_utc->tm_min, tm_utc->tm_sec);
	t2 = mktime(tm_utc);
	DPRI_INFO("***\n");
	DPRI_INFO("t1=%ld\nt2=%ld\n", t1, t2);
	DPRI_INFO("%ld\n", (t1 - t2));
...
int fzb_SetDevCollectLogFilesFunc(unsigned char *pucPeerId,unsigned int iSessionId,unsigned char *pcDesInfo){
	...
	if( access(DEVICE_CONFIG_PATH,F_OK) == 0 ){
		...
		if(state != 0){
			DPRI_INFO("system err!state:%d\n",state);
			return -1;
		}
	}else{
		DPRI_INFO("accessDEVICE_CONFIG_PATH err!\n");
		return -1;
	}	
	ZJ_SetCollectLogFilesStatus( pucPeerId, iSessionId, state, pcDesInfo, (unsigned char *) acFileName);
	DPRI_INFO("SetDevCollectLogFilesFunc succeed!\n");
	return 0;
}
