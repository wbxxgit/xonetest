...
int fzb_GetSDCardInfo(unsigned int *piTotalSize, unsigned int *piFreeSize){  
	...
	ZJ_LOG_INF("ret1 ,total:%lld,free:%lld,USED:%lld",info2.a_total_size,info2.a_free_size,info2.a_used_size);
	*piTotalSize = info2.a_total_size/1024;
	*piFreeSize = info2.a_free_size/1024;
	ZJ_LOG_INF("retsdd ,total:%lld,free:%lld",*piTotalSize,*piFreeSize);
	return 0;
#if 0	
	FILE *fstream = NULL;
	char buff[1024];	
