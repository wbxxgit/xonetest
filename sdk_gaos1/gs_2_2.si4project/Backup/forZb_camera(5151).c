...
static int SDK_SnapShot()
{
	...
	ZJ_LOG_INF("SDK_SnapShot pic_path >>>>>>>>>>%s\n",un_snap_path.pic_path);
...
int fzb_VideoGetJpeg(EN_ZJ_PICTURE_TYPE enPicType, unsigned char **ppucJpegBuf){
	...
	ZJ_LOG_INF("GetJpeg callback enPicType:%d,ppucJpegBuf:%p\n",enPicType,*ppucJpegBuf);
	if( SDK_SnapShot() != 0){
...
//			ZJ_LOG_INF("%s",buff);	
	...
	memcpy(buff_int,buff,(strlen(buff)-2));
	ZJ_LOG_INF("%s",buff_int);

	*piTotalSize = atoi(buff_int);
	
	memset(buff,0,sizeof(buff));
	memset(buff_int,0,sizeof(buff_int));
	if(NULL == ( fstream = popen("df -m|grep sd|awk '{print $5}'","r"))){	
			fprintf(stderr,"execute command failed:%s",strerror(errno));
			return -1;
	}	
//	while( NULL != fgets(buff,sizeof(buff),fstream)){
