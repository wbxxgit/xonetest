...
void *pthread_waitQRandAPst(void *arg){
	...
	while(--num_int != 0){
		...
		if( (unique_dwifiMsg.netState &0x02) == 0x02 ){
...
int d_netStateSetPro(EN_dev_netType netTP){


	
	if( (netTP == EN_D_NTYPE_NORWIFI) ||(netTP == EN_D_NTYPE_WIREDNET)){

	}else if(  (netTP == EN_D_NTYPE_NONET) ||(netTP == EN_D_NTYPE_QLINK) ){
		setSdkNetType(EN_ZJ_NETWORK_TYPE_NONET);
	}else{
