...
#define TUTK_PLATFORM			0		//TUTKÆ½Ì¨ºê¶¨Òå¿ª¹Ø
...
char *myStrncpy(char *dest, const char *src, int n) 
{
    ...
    char *tmp = (char*)malloc(size);  // å¼€è¾Ÿå¤§å°ä¸ºn+1çš„ä¸´æ—¶å†…å­˜tmp
    if (tmp) 
    {
        memset(tmp, '\0', size);  // å°†å†…å­˜åˆå§‹åŒ–ä¸º0
        memcpy(tmp, src, size - 1);  // å°†srcçš„å‰nä¸ªå­—èŠ‚æ‹·è´åˆ°tmp
        memcpy(dest, tmp, size);  // å°†ä¸´æ—¶ç©ºé—´tmpçš„å†…å®¹æ‹·è´åˆ°dest
...
static unsigned int lin_GetTickCount()
{
...
//	if(SegCam_getVideoSw() != ALLOW_VIDEOSTREAM)

	if(SegCam_getVideoArrSw_cmpRet(nChn) != 1 )
		return -2;	 //sw off
//	printf("after SegCam_getVideoSw\n");	
	...
	if(0 == p->type || 1 == p->type)
	{
		...
		{
			gk_header.frame_type = 2;  //GK_NET_FRAME_TYPE_P
		}
	}	
	gk_header.sec = p->t_time;
	gk_header.pts = p->pts;
...
/*	if(0 == nChn)
		dj_video_GtFTbl()->f_Video_WriteFrame(0,data,frame_size,lin_GetTickCount(),gk_header.frame_type);
	else	
		dj_video_GtFTbl()->f_Video_WriteFrame(1,data,frame_size,lin_GetTickCount(),gk_header.frame_type);*/	
	...
	dj_video_GtFTbl()->f_Video_WriteFrame(nChn,data,frame_size,lin_GetTickCount(),gk_header.frame_type);
...
int AV_FarmeCallback_2(T_SDK_STREAM_CALLBACK_PARAM *encodeData)	//PCM Êý¾Ý»Øµ÷º¯Êý
{ 
	   if(NULL == encodeData)
		{
			printf("T_SDK_STREAM_CALLBACK_PARAM is NULL\n");
...
static int alarmcb_process(E_SDK_ALARM_TYPE alarmType){
	int i;
	ZJ_LOG_INF("alarmExe_memSize() = %d",alarmExe_memSize());
	for (i = 0; i < alarmExe_memSize(); ++i){
		
	/*    if(((*(getAlarmArr+i))->al_type) == 1){
			(*getAlarmArr)[i].pfun();
			ZJ_LOG_INF("has been alarmcb_process");
			return 0;
		}	*/

		if(    gAlarmExe[i].al_type == alarmType){
			gAlarmExe[i].pfun();
			ZJ_LOG_INF("has been alarmcb_process");
			return 0;			
		}
...
int AlarmCallback(T_SDK_ALARM_CALLBACK_PARAM *param)
{
	...
	// zj motion alarm
	ZJ_LOG_INF("param->e_alarm_type = %d\n",param->e_alarm_type);
/*	if(param->e_alarm_type == E_VIDEO_MOTION){  
	...
	//alarm array process
	if(0 != alarmcb_process(param->e_alarm_type))
	...
	#endif
	{
		if(NULL != func_AlarmCallback)
		{

			#if GOS_PERSON_BODY_DETECT
			if(param->e_alarm_type == E_BODY_DETECT)
				param->e_alarm_type = 3;   //3 Îª21cn¹æ¶¨µÄÈËÁ³Õì²â±¨¾¯ÀàÐÍ
			#endif
			
			T_SDK_PIC_PATH un_snap_path;
			memset(&un_snap_path, 0, sizeof(un_snap_path));
			strcpy(un_snap_path.pic_path, "/tmp/21cn_motion.jpg");
			char strTmpAlarmFile[256] = {0};
			int utc_time = 0;
			Utils_AlarmReName(strTmpAlarmFile, sizeof(strTmpAlarmFile), &utc_time);	
			if (0 != SDK_Cmd_Impl(SDKCMD_SET_SNAPSHOT_PATH,(void *)&un_snap_path))
			{
				 printf("callSDKCMD_SET_SNAPSHOT_PATH fail .\n");
				 unlink(un_snap_path.pic_path);
				return -1;
			}
			
			struct stat file_info;	
			stat(un_snap_path.pic_path, &file_info);
			FILE * hd_src = fopen(un_snap_path.pic_path, "rb");
			char alarmPic_buf[600 * 1024] = {0};

			if(file_info.st_size <= sizeof(alarmPic_buf))
			{
				fread(alarmPic_buf, file_info.st_size, 1, hd_src);
				func_AlarmCallback(param->e_alarm_type, alarmPic_buf, file_info.st_size, 0, strTmpAlarmFile, NULL, (time_t)utc_time);
			}
...
//	int para;
    ...
    //sleep(5);//ÏµÍ³ÅÜÆðÀ´ ´ó¸ÅÐèÒª5Ãë  
	testProce_native();

			
	zjsdk_init();
	zjsdk_start();
	printf("zjsdk_start going on\n");	
