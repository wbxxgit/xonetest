...
int fzb_SetDeviceRebootCbFunc(){
	ZJ_LOG_INF("SetDeviceRebootCbFunc exe");
	system("reboot");
	return 0;
}
...
//	t_ntp_cfg.a_NtpServer = "";	
	...
	strcpy(t_ntp_cfg.a_NtpServer,"server 3.cn.pool.ntp.org");
	t_ntp_cfg.un_ntp_port = 123;
//	t_ntp_cfg.un_Res[0] = SyncFlag;
	ZJ_LOG_INF("un_TimeZone:%d",t_ntp_cfg.un_TimeZone);
	if(0 != SDK_Cmd_Impl(SDKCMD_SET_NTP_PARAM,  &t_ntp_cfg) ){
		ZJ_LOG_INF("SDKCMD_SET_NTP_PARAM fail");
		return -1;
	}
	return 0;
}
...
static int getZone_ntp(int *timezone,int *syncflag){
	...
	if(0 != SDK_Cmd_Impl(SDKCMD_GET_NTP_PARAM,  &t_ntp_cfg) ){
		ZJ_LOG_INF("SDKCMD_GET_NTP_PARAM fail");
		return -1;
	}	
	printf("getZone_ntp:\n");
...
int fzb_funSetTimeZone(int iSyncFlag, int iZone, char* pcDatetime){
	ZJ_LOG_INF("b_funSetTimeZone_qx exe,syncflag [%d] zone [%d] datetime [%s]",iSyncFlag, iZone, pcDatetime);
	...
	if(iSyncFlag == 0){
		...
		if( EOF == 	sscanf( pcDatetime, "%d-%d-%d %d:%d:%d", &dtTmp.un_year, &dtTmp.un_month, &dtTmp.un_day,
			...
			return -1;
		ZJ_LOG_INF("switch:%d,%d,%d,%d,%d,%d", dtTmp.un_year, dtTmp.un_month, dtTmp.un_day,
				dtTmp.un_hour,dtTmp.un_minute,dtTmp.un_second );
		if (0 != SDK_Cmd_Impl(SDKCMD_SET_DEVICE_TIME,(void *)&dtTmp))
		{
			ZJ_LOG_INF("SDKCMD_SET_DEVICE_TIME fail");
			return -1;
		}
	}	
...
int fzb_funSetTimeZone_PZJ(int iSyncFlag, int iZone, char* pcDatetime,char *pucDst){
	ZJ_LOG_INF("b_funSetTimeZone_PZJ exe, syncflag [%d] zone [%d] datetime [%s] pucDst [%s]", 
...
//	*piZone=	getZone();//getZone_ntp
//	*piZone=	getZone_ntp();
	if(0 != getZone_ntp(piZone,piSyncFlag))
		return -1;
//	*piSyncFlag = inter_getSyncFlag();	
