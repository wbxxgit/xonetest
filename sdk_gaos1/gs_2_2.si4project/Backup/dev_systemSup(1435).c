...
static int ParaInit_wifiMsg(char *path,char *file){
	...
	//	if( EOF != fscanf(pFile,"SyncFlag:%d",&time_syncFlag)){
		...
		//	printf("read flag %d,%s,%s\n",OwifiSsidMsg.flag,(OwifiSsidMsg.ssid[0]),(OwifiSsidMsg.pwd[0]));
			printf("read flag %d\n",OwifiSsidMsg.flag);
			fclose(pFile);	
		}else{
...
static void *dev_systemSup_loop(){	
	...
	while(1){
		...
		if(getipJudge() == 0)
			printf("systemSup getipJudge succeed\n");
		else
			printf("systemSup getipJudge fail\n");
	...
	//	i_sdM++
		if(i_sdM++%5 == 0){
			...
			if(retcode != 0)			
				ZJ_LOG_INF("SDKCMD_GET_SD_CARD_MOUNT error ! \n");
			else{
				...
				if(un_connected == 3){  //mount
					...
					if(glo_sdMountState == 3){	//trigger
						glo_sdMountState = 1;
						ZJ_LOG_INF("unnount  trigger! \n");
						zjre_packF_sdMountPro(0);					
					}
					
				}
			
			//	ZJ_LOG_INF("SD Card check Mount = %d",un_connected);	
			}
