...
//	printf("nEncType:%d,nChn:%d,gk_header.pts:%d,gk_header.frame_type:%d\n",nEncType,nChn,gk_header.pts,gk_header.frame_type);
	if(1 == nEncType){
		if(0 == nChn)
	...
	#endif
	{
		if(NULL != func_AlarmCallback)
		{
			...
			if(param->e_alarm_type == E_BODY_DETECT)
				param->e_alarm_type = 3;   //3 为21cn规定的人脸侦测报警类型
			#endif
